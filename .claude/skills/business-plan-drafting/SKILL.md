...
- ❌ Citing any of `ASM-015`–`028` as `Approved` when `Assumptions_Register.md` still shows them
  `Proposed` — cite the status honestly, or route through the approval step first.
...
- ❌ Representing the OS's capital-allocation recommendation as an automatic management decision —
  human/instructor approval of major reallocations is mandatory (the Responsible-AI principle Section 11
  must operationalize).
