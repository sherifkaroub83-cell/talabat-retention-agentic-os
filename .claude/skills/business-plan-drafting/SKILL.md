...
**Draw from:** `Topics/AI.md`, `Topics/Recommendation Systems.md`, `Entities/Technology_Platforms.md` (unchanged — talabat's Delivery Hero-licensed stack remains the citable build/buy/partner anchor) · `Strategic/AI Opportunities.md` (talabat's own stated roadmap — evidenced intent, distinct from Section 12's proposed roadmap) · `vault/Forecasts/Value_Driver_Tree_v2.md` nodes N-06 (AI/personalisation as a cross-cutting capability embedded in *both* 2026 investment buckets — the clearest evidence for how this capability actually fits the current programme; the node IDs live in `Value_Driver_Tree_v2.md`, never in `Investment_Relationship_Map.md`, which provides the narrative provenance but carries no numbered nodes — corrected 2026-07-24 after Pass 2 audits of Sections 4/5 traced mis-cites to this entry) and N-09 (the capability's own disclosed EBITDA contribution, a distinct node from N-06).
**Note on MCP:** resolved 2026-07-24 by `DEC-011` (pursue) — the OS runs a read-only `vault-mcp` server (`scripts/vault_mcp/server.py`, registered in `.mcp.json`; query-only by design, so it cannot bypass pipeline gates). Describe this actual tooling; do not overstate it beyond read-only status.
**Caution — new:** N-06 is explicitly *not* named as a separate dollar line within either the ~USD120mn or ~USD55mn bucket (`Investment_Relationship_Map.md`) — do not invent an "AI budget" figure. Describe it as embedded infrastructure whose own EBITDA contribution is company-estimated (USD14mn+ FY2024 → USD30mn+ FY2025, `Facts/AI_Facts.md` / `Topics/AI.md`, per TLB-001 p.23 / TLB-002 p.16, Group level — NOT `Topics/EBITDA.md`, which does not carry these figures; mis-pointer corrected 2026-07-24 after Pass 2 audits), while its share of either 2026 bucket specifically is not.
